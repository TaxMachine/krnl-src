...
public class krnl_monaco : Form
{
	...
	private void Form1_Load(object sender, EventArgs e)
	{
		...
		if (Directory.GetFiles("bin/tabs").Length != 0)
		{
			...
			{
				int num = customTabControl1.TabPages.Count - 2;
				int num2 = j;
				using FileStream stream = new FileStream($"bin/tabs/{num2}_source.lua", FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
			...
			for (int k = 0; k < Directory.GetFiles("bin/tabs").Length / 2; k++)
			{
				...
				customTabControl1.GetWorkingTextEditor().add_LoadingStateChanged((EventHandler<LoadingStateChangedEventArgs>)delegate(object sender, LoadingStateChangedEventArgs args)
				{
					if (!args.get_IsLoading())
					{
						try
						{
