...
internal static class Program
{
	...
	public static bool hasFolder(string name, string path)
	{
		...
		{
			if ((dynamic)directories[i].Name == name)
			{
				return true;
			}
		}
	...
	private static void Main()
	{
		...
		while (stack.Count != 0)
		{
			if ((dynamic)string.Join("\\", stack.ToArray().Reverse()) + "\\" == Path.GetTempPath())
			{
				flag = true;
				break;
			}
			...
			new Thread((ThreadStart)delegate
			{
				while (true)
				{
					...
					if (krnl_monaco.findpipe("krnlpipe"))
					{
						...
						{
							(form.Controls["bunifuFlatButton5"] as BunifuFlatButton).Text = "INJECTED";
						});
						injecting = false;
						failed_inject = false;
					}
					else if (!injecting || failed_inject || (Process.GetProcessesByName("RobloxPlayerBeta").Length == 0 && Process.GetProcessesByName("RobloxPlayerLauncher").Length == 0))
					{
						form.Invoke((MethodInvoker)delegate
						{
							(form.Controls["bunifuFlatButton5"] as BunifuFlatButton).Text = "INJECT";
						});
						injecting = false;
						failed_inject = false;
					}
